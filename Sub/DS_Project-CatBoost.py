...
val_pool = Pool(_encode_dates(_encode_covid(_gas_price_encoder(_merge_external_data(X)))), label=y, cat_features=categorical_cols)
pipe.fit(X, y, regressor__cat_features=categorical_cols, regressor__early_stopping_rounds=70, regressor__eval_set=val_pool)
