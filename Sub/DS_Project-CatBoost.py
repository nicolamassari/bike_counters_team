...
def add_lags(X, cols_to_lag=['t', 'u', 'vv', 'nnuage4'], lag_list=[2, -24, -2]):
    ...
    for lag in lag_list:
        ...
        X[lag_columns] = X[lag_columns].interpolate(method='linear'). \
...
def add_moving_average(X, cols_to_ma=['t', 'u', 'vv', 'nnuage4'], window_list=[24*7, 24], centered=True):
    ...
    for w in window_list:
        ...
        X[ma_columns] = X[ma_columns].interpolate(method='linear'). \
            interpolate(method='bfill').interpolate(method='ffill')
...
def _merge_external_data(X, include_lags=True, include_ma=True):
    to_keep = [
        'date', 'hnuage4', 't', 'ctype4', 'nnuage4', 'u',
        'etat_sol', 'perssfrai', 'tx12', 'cm', 'tn12',
        'tend24', 'vv', 'rafper', 'rr24', 'hnuage2', 'td',
        'rr3', 'hnuage3', 'hnuage1'
    ]
    ...
    ext_data[columns_to_interpolate] = ext_data[columns_to_interpolate]. \
        interpolate(method='polynomial', order=3).interpolate(method='bfill').interpolate(method='ffill')
...
data.drop(
    columns=['site_name', 'counter_id', 'site_id', 'counter_installation_date', 'coordinates', 'counter_technical_id'],
    inplace=True
)
...
val_pool = Pool(_encode_dates(_encode_covid(_gas_price_encoder(_merge_external_data(X)))),
                label=y, cat_features=categorical_cols)
pipe.fit(
    X,
    y,
